...
func (p *OBJParser) cleanUp() {
	p.positions = make([]mgl32.Vec3, 0)
	p.texCoords = make([]mgl32.Vec2, 0)
	p.normals = make([]mgl32.Vec3, 0)
...
func (p *OBJParser) ParseFile(filename string) (*Model, error) {
	...
	p.positions = append(p.positions, mgl32.Vec3{0, 0, 0})
	p.texCoords = append(p.texCoords, mgl32.Vec2{0, 0})
	p.normals = append(p.normals, mgl32.Vec3{0, 0, 0})
	...
	for scanner.Scan() {
		...
		case "v":
			...
			p.positions = append(p.positions, mgl32.Vec3{float32(x), float32(y), float32(z)})
		...
		case "vt":
			...
			p.texCoords = append(p.texCoords, mgl32.Vec2{float32(u), float32(v)})
		...
		case "vn":
			...
			// Нормализуем нормаль
			normal := mgl32.Vec3{float32(nx), float32(ny), float32(nz)}.Normalize()
